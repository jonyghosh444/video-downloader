        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                for f in formats:
                    ...
                    resolution = f.get('resolution', f.get('height', 'N/A'))  # Fallback to height if resolution is missing
                    format_note = f.get('format_note', 'N/A')

                    format_label = f"{format_id} - {resolution} - {format_note}"
                    format_options.append((format_label, format_id))  # Tuple (label, format_id)

                st.session_state.available_formats = format_options  # Store in session state
                st.write("Formats loaded successfully! Please select a format from the dropdown below.")
...
if st.session_state.available_formats:
    ...
    if selected_format_label:
        st.session_state.selected_format_code = format_codes[format_labels.index(selected_format_label)]
    ...
    if video_url and st.session_state.selected_format_code:
        ...
        with st.spinner("Downloading... Please wait."):
            try:
                ...
                st.success(f"Download complete! Video saved in '{DOWNLOAD_DIR}' folder.")
            except Exception as e:
